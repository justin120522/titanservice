...
export default function ProfilePage() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (user) {
      const updatedUser: AuthUser = { ...user, name: form.name, email: form.email, phone: form.phone, address: form.address };
    ...
    <div className="max-w-2xl animate-fade-in-up">
      ...
      <div className="glass-card mb-6">
        <div className="flex items-center gap-5">
          <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-primary to-accent flex items-center justify-center text-white text-2xl font-bold shadow-lg shadow-primary/20">{initials}</div>
          <div>
            <h3 className="text-lg font-semibold text-text-primary">{form.name || "Your Name"}</h3>
            <p className="text-sm text-text-muted mb-3">Customer · Member since {memberDate}</p>
            <button type="button" className="btn btn-glass btn-sm">Change Avatar</button>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={handleSave} className="space-y-6">
        <div className="glass-card">
          <h3 className="text-lg font-semibold text-text-primary mb-5">Personal Information</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input type="text" value={form.address} onChange={(e) => update("address", e.target.value)} className="glass-input" />
            </div>
          </div>
        </div>
